...
Figures are written directly below ``results/figs/<CATEGORY>/`` and data below the
parallel ``results/data/analysis/<CATEGORY>/<script_name>/`` tree.  A run manifest is written
...
def _output_root(kind: str) -> Path:
    ...
    if kind == "figs":
        return PROJECT_ROOT / "results" / "figs"
    return PROJECT_ROOT / "results" / "data" / "analysis"
