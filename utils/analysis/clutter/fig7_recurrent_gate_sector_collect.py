"""Collect equal-n recurrent gate/activation caches for all nine sector contexts.

The complete test split is streamed once and a reproducible random subset is retained from each
sector.  The retained count is the smallest observed sector-frame count, matching Figure 6's
sector protocol.  Each ``sector{s}_gate_act_cache.npz`` retains the established gate-cache
schema plus sampling metadata.
"""
...
def _reference_tuned_masks(cache_dir: Path) -> np.ndarray:
    """Load the previously validated sector-specific ``T_old`` masks before replacement."""

    masks: list[np.ndarray] = []
    for sector in SECTORS:
        path = cache_dir / f"sector{sector}_gate_act_cache.npz"
...
def main() -> None:
    ...
    selected, target, original_counts = _equal_n_condition_mask(
        all_labels[:, SECTOR_LABEL_COL], len(SECTORS), cli.selection_seed
    )
    print(f"dataset sequences={len(dataset)}, hidden_size={hidden_size}, "
          f"sectors={SECTORS}, equal_n/sector={target}, "
    ...
    if T_old_all.shape != (len(SECTORS), hidden_size):
        raise RuntimeError(f"Unexpected sector tuned-mask shape: {T_old_all.shape}")
    print(
        f"T_old sizes per sector: {[int(T_old_all[s].sum()) for s in SECTORS]}",
        flush=True,
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            for time_idx in range(frame_num):
                ...
                for s in SECTORS:
                    mask = batch_selected[:, time_idx] & (sector_labels == s)
                    if mask.any():
    ...
    if frame_offset != total_frames or any(counts[s] != target for s in SECTORS):
        raise RuntimeError(
            f"Equal-n sector accumulation mismatch: expected {target}, got {counts}."
        )

    for s in SECTORS:
        if not gate_chunks[s]:
            raise RuntimeError(f"sector {s}: no frames collected")
        G_flat = np.concatenate(gate_chunks[s], axis=0)
        act_flat = np.concatenate(act_chunks[s], axis=0)
