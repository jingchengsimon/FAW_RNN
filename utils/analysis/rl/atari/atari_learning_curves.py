"""Plot Atari DQN learning curves for named Pong protocols, sweeps, or one multi-task run.

Reads ``results/data/rl/atari/runs/<suffix>/metrics_history.jsonl`` (written by
``run_task.py atari-dqn``) and overlays ``episodic_return_100`` vs ``global_step``
...
the shared plotting conventions in this module (matplotlib Agg, fixed per-model
...
Examples:
  ...
  python -m utils.analysis.rl.atari.atari_learning_curves --setting flicker --smooth 20
  python -m utils.analysis.rl.atari.atari_learning_curves --run_dir results/data/rl/atari/runs/<suffix>
  python -m utils.analysis.rl.atari.atari_learning_curves --run_dir results/data/rl/atari/runs/<suffix> \
      --include_combined
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data_root", default="results/data/rl/atari/runs")
    parser.add_argument(
        "--output_dir",
