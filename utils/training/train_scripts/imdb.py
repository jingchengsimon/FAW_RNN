...
A self-contained text entry point parallel to ``utils.training.train_scripts.clutter``. It reuses the
lightweight helpers (``set_seed``) and the text subsystem (``utils.training.text.text_imdb_data``,
``utils.training.text.text_task_models``) but keeps its own compact train loop because the vision
...
under ``results/data/text/runs/<result_suffix>/``.

Example (local smoke)::

    python run_task.py imdb --model_types lstm gawf --hidden_sizes 128 \
...
from utils.training.text.text_imdb_data import build_imdb_loaders, load_meta
from utils.training.text.text_task_models import get_text_model_classes
from utils.training.recurrent_cores import configure_gawf_feedback_acceleration
from utils.training.text.text_train_utils import (
    build_optimizer,
    count_core_params,
    maybe_subset,
...
def save_outputs(args, cfg: Dict, out: Dict) -> str:
    ...
    result_dir = os.path.join(results_root, "data", "text", "runs", args.result_suffix)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--data_dir", default=None, help="Base data dir (CLI -> env -> <repo>/source/text/data).")
    ...
    p.add_argument(
        "--result_suffix", default="imdb_hparam", help="Subdir under results/data/text/runs/."
    )
