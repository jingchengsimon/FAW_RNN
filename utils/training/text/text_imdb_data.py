...
exposes them to ``run_task.py imdb``. Compute jobs load everything offline; no
...
from ..common import worker_init_fn
...
def resolve_data_dir(cli_data_dir: Optional[str]) -> str:
    """Resolve Text data (CLI -> env -> <repo>/source/text/data)."""
    if cli_data_dir:
    ...
    return os.path.join(repo, "source", "text", "data")
