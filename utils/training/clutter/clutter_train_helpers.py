...
class PathHelper:
    ...
    def get_base_path(
        ...
        base_path = os.path.join(_repo_root, "source", "clutter", "stimuli")
        if logger is not None:
...
def pick_cuda_device_index_prefer_no_python() -> Optional[int]:
    ...
    returned index (same pattern as ``run_task.py clutter``).
    """
    indices = _enum_cuda_device_indices()
    if not indices:
        return None
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/data/clutter/runs/<result_suffix>/gawf_diagnostics/."
        ),
    )
    parser.add_argument(
        "--seed",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If not set, uses AIM3_STIMULI_PATH / FAW_RNN_DATA_PATH env, else "
            "<repo>/source/clutter/stimuli."
        ),
    )
    parser.add_argument(
        "--results_dir",
        ...
        default="",
        help=(
            "Root directory under which results/data/clutter/runs is written. "
