...
```bash
python -m remote.monitoring.job_registry new \
...
```bash
python -m remote.monitoring.job_registry register /tmp/my-run-manifest.json
```
...
python -m remote.monitoring.progress

# 任何稳定标识均可搜索
python -m remote.monitoring.progress 58145944
python -m remote.monitoring.progress fscompare1m
python -m remote.monitoring.progress sjc-pong-fscompare1m-c219996

# 一次检查某 host 的所有 active jobs
python -m remote.monitoring.progress --active --host amarel

# 机器可读结果
python -m remote.monitoring.progress fscompare1m --json
```
...
```bash
python -m remote.monitoring.progress --active --ssh-alias sjc-remote=my-sjc-alias
```

## 状态、同步与保留

```bash
python -m remote.monitoring.job_registry list
python -m remote.monitoring.job_registry list --active
python -m remote.monitoring.job_registry show 58145944
python -m remote.monitoring.job_registry set-status <id> running
python -m remote.monitoring.job_registry rebuild
```
...
```bash
python -m remote.monitoring.job_registry remove <id> --human-confirmed
```
