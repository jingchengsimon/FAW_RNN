"""Plot four-group recurrent SOURCE-gate distributions per context on a single grid.

Reads ``recurrent_group_gate_distributions.npz`` produced by
``utils_anal.gawf_recurrent_group_gate_distributions`` and renders one grid figure per
factor: ``recurrent_sector`` (3x3) and ``recurrent_digit`` (2x5). Each panel shows the four
per-context distributions ``within_top``, ``top_to_rem``, ``rem_to_top``, ``within_rem``
with mean lines and Cohen's d annotations comparing every within-group distribution to its
paired between-group counterpart (same source group).
"""
...
GROUP_LABELS = {
    "within_top": "Top10 → Top10 (within-top)",
    "top_to_rem": "Top10 → Rem (source top, dest rem)",
    "rem_to_top": "Rem → Top10 (source rem, dest top)",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=str(
            ...
            / "recurrent_group_gate_distributions.npz"
        ),
    )
    parser.add_argument(
        "--fig_dir",
        default=str(
            output_dir("E_relevance_alignment", "gawf_recurrent_group_gate_distributions", "figs")
            / "efferent"
        ),
    )
...
def plot_group_panel(
    ...
    *,
    ...
    """Draw one context's four-group SOURCE-gate density comparison onto ``axis``.
    ...
    (within_top vs top_to_rem, within_rem vs rem_to_top).
    """
    ...
    axis.set_xlabel(
        "Recurrent gate averaged over destination subset",
        fontsize=label_font,
    )
    ...
    axis.set_title(
        f"{factor_label} {context} — four-group SOURCE-gate distribution\n"
        f"d(within_top vs top→rem)={d_top:.3f}; d(within_rem vs rem→top)={d_rem:.3f}",
        fontsize=title_font,
    )
...
def render_grid(
    ...
    """Render every context of one factor into a single grid figure."""
    ...
    fig.tight_layout()
    destination = fig_dir / f"{cell}_source_dest_group_split_distribution_grid.png"
...
def main() -> None:
    """Load saved histograms and render the two grid figures."""
