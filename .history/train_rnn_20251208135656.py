...
class RNNConv(nn.Module):
    ...
    def middle(self, x):
        ...
        x = self.LNormRNN(x)
        x = F.relu(x)
        x = F.dropout(x, p=0.4, training=self.training)  # Reduced from 0.5 to 0.4 to balance overfitting/underfitting
        return x

    def classifier(self, x):
        # Add dropout before classifier to prevent overfitting, especially for character task
...
class GaWFRNNConv(nn.Module):
    ...
    def middle(self, x, feedback=None):
        ...
        # Dropout
        x = F.dropout(x, p=0.45, training=self.training)  # Increased to 0.45 to further reduce overfitting for character task
        return x
...
def network_train(mdl, train_data, val_data, num_epochs=50, loss_weights=None, lr=0.001, 
    ...
    # Add learning rate scheduler to help find better generalization point
    ...
    # Use label smoothing to prevent overconfidence, especially for character task
    criterion_char = nn.CrossEntropyLoss(label_smoothing=label_smoothing)
    
    # Select position loss function based on use_sector
    if use_sector:
        criterion_pos = nn.CrossEntropyLoss(label_smoothing=label_smoothing)  # sector classification
    else:
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            # Update learning rate scheduler based on validation performance
            if use_sector:
            ...
            # Print learning rate change if it decreased
            if new_lr < old_lr:
...
if __name__ == "__main__":
    ...
    dropout_rate = 0.3  # CNN encoder dropout rate
    mdl_rnn = ModelClass(num_classes=10, num_pos=num_pos, kernel_size=5, dropout_rate=dropout_rate)
    model_suffix = model_type
    print(f"Using model type: {model_type.upper()} (num_pos={num_pos}, dropout_rate={dropout_rate})")
    ...
    results_rnn = network_train(
        ...
        weight_decay=3e-4,  # L2 regularization, increased to further reduce overfitting for character task
        dropout_rate=0.45,  # CNN encoder dropout rate, increased to further reduce overfitting
        use_early_stopping=True,  # Enable early stopping to find best generalization point
        early_stopping_patience=5,  # Reduced patience to stop earlier when character val acc plateaus (around epoch 60-80)
        min_delta=0.001,  # Early stopping minimum improvement threshold
        label_smoothing=0.1  # Label smoothing to prevent overconfidence, especially for character task
    )
    ...
    # File naming includes both mode and model type for easy distinction of different experiments
    # e.g., results_rnn_sector.pkl, results_lstm_coord_acc.pkl
    results_rnn_path = f"results_{model_suffix}_{mode_suffix}{acc_suffix}"
