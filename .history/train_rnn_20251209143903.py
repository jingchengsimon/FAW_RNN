...
def network_train(mdl, train_data, val_data, num_epochs=50, loss_weights=None, lr=0.001, 
    ...
    if use_modification:
        ...
        if use_early_stopping is None:
            use_early_stopping = True  # Default early stopping
...
if __name__ == "__main__":
    ...
    # Note: dropout_rate needs to be set when creating the model, so we handle it here
    # weight_decay and use_early_stopping are handled inside network_train function
    use_modification = False  # Set to True to enable regularization modifications, False to disable
    ...
    # network_train will handle weight_decay and use_early_stopping based on use_modification
    # You can still override with custom values if use_modification=True
    results_rnn = network_train(
        ...
        use_modification=use_modification,  # Control modification settings
        # Optional: override defaults when use_modification=True
        # weight_decay=1e-4,  # Custom weight_decay (only used if use_modification=True)
        # dropout_rate=0.3,  # Custom dropout_rate (only used if use_modification=True, for reference only)
        # use_early_stopping=True,  # Custom use_early_stopping (only used if use_modification=True)
    ...
    results_rnn_path = f"results_{model_suffix}_{mode_suffix}{acc_suffix}_256"
