...
# ==================== 加速训练模块（可选） ====================
# 这些模块只有在 use_acceleration=True 时才会被使用
# 默认情况下（use_acceleration=False）不会导入，保持代码简洁

def _init_acceleration_modules():
    """初始化加速训练相关的模块（仅在需要时导入）"""
    try:
...
class GaWFRNNConv(nn.Module):
    """
    GaWF (Gated with Feedback) RNN 模型
    
    主要改进：
    1. 将 classifier 的输出作为 feedback 反馈到 RNN 的输入中
    2. Feedback 经过 U @ diag(concat) @ V 变换后，与 RNN 权重做 Hadamard product
    """
...
def network_train(mdl, train_data, val_data, num_epochs=50, loss_weights=None, lr=0.001, 
    ...
    # 根据 use_sector 返回不同的键名
    if use_sector:
        return {
            "train_acc_char": train_acc_char,
            "val_acc_char": val_acc_char,
            "train_acc_pos": train_metric_pos,  # sector 准确率
            "val_acc_pos": val_metric_pos,      # sector 准确率
            "model": mdl.to("cpu")
        }
    else:
        return {
            "train_acc_char": train_acc_char,
            "val_acc_char": val_acc_char,
            "train_err_pos": train_metric_pos,  # 坐标 MSE
            "val_err_pos": val_metric_pos,      # 坐标 MSE
            "model": mdl.to("cpu")
        }
...
if __name__ == "__main__":
    ...
    mdl_rnn = ModelClass(num_classes=10, num_pos=num_pos, kernel_size=5)
    model_suffix = model_type
    print(f"使用模型类型: {model_type.upper()} (num_pos={num_pos})")
