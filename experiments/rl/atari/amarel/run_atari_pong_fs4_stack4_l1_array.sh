#!/usr/bin/env bash
#SBATCH --job-name=aim3-pong-fs4s4-l1
...
# Strict 6-action Pong sweep:
# 7 models x 2 observation settings x 5 seeds = 70 tasks.
# The protocol is intentionally fixed to frame_skip=4 and frame_stack=4.
...
: "${MATCH_JSON:?MATCH_JSON must point to the L1 parameter-match JSON}"
...
SEQ_LEN="${SEQ_LEN:-16}"
RUN_TAG="${RUN_TAG:-pong_fs4_stack4_l1}"
ARTIFACT_TAG="${ARTIFACT_TAG:-atari_pong_fs4_stack4_l1}"
...
echo "protocol=6-action-minimal frame_skip=4 frame_stack=4 layers=1 flicker=$FLICKER_PROB"
...

{
  ...
  echo "frame_skip=4 frame_stack=4 layers=1 action_space=minimal num_actions=6"
