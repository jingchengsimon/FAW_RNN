...
load_remote_config() {
  ...
  : "${REMOTE_HOST:?REMOTE_HOST must be set in experiments/remote/config.sh}"
  : "${REMOTE_PROJECT_DIR:?REMOTE_PROJECT_DIR must be set in experiments/remote/config.sh}"
...
remote_body_with_activation() {
  ...
  if [[ -n "${REMOTE_ACTIVATE}" ]]; then
    printf "%s && export PYTHONDONTWRITEBYTECODE=1 && %s" "${REMOTE_ACTIVATE}" "${body}"
  else
    printf "export PYTHONDONTWRITEBYTECODE=1 && %s" "${body}"
  fi
}
