...
# shellcheck source=experiments/remote/lib.sh
source "${SCRIPT_DIR}/lib.sh"

usage() {
  cat <<'EOF'
Usage: ./experiments/remote/run.sh [options] -- <command> [args...]
...
Examples:
  ...
  ./experiments/remote/run.sh --detach hparam_4h -- bash experiments/<task>/amarel/check_hparam_4h_5epoch_test_status.sh
  ./experiments/remote/run.sh --push -- bash experiments/<task>/amarel/run_hparam_full_grid_2gpu.sh --scale 4
...
if [[ "${detach}" == true ]]; then
  ...
  echo "  ./experiments/remote/fetch_results.sh --since ${marker_name}"
  exit 0
fi
