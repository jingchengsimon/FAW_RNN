#!/usr/bin/env bash
# Compute the ten-seed digit-conditioned encoder activation patterns for Figure 6 on SJC.
...
OUT_DEFAULT="$RESULTS/data/analysis/fig6_encoder_digit_patterns_gawf_10seed"
OUT="${AIM3_FIG6_ENCODER_DIGIT_PATTERNS_OUT:-$OUT_DEFAULT}"
...
for seed in $(seq -w 1 10); do
  ...
  if [[ -e "$SEED_OUT" ]]; then
    [[ -f "$SEED_OUT/encoder_digit_patterns.npz" && -f "$SEED_OUT/manifest.json" ]] || {
  ...
  python -m utils.analysis.clutter.fig6_encoder_sector_patterns collect \
    ...
    --device cuda --batch_size 16 --num_workers 2 --data_suffix 40h-uint8 --condition digit
done
...
python -m utils.analysis.clutter.fig6_encoder_sector_patterns plot \
  --data_root "$OUT" --figure_dir "$FINAL" --condition digit
