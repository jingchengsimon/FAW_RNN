"""Export recurrent AFFERENT gate distributions split by source AND destination group.

The literal user-facing framing of the top-10% recurrent gate distribution is afferent:
for every hidden unit ``h``, the distribution shows the average of gates that arrive at ``h``
from all effective source units (``mean_{i in eligible} G[h, i]``). This module splits that
one afferent value per unit into two values per unit, one per source group:

- ``m_from_top[h] = mean_{i in top10(context)} G[h, i]``
- ``m_from_rem[h] = mean_{i in remaining_eligible} G[h, i]``

Combined with the destination unit's own top-10% group membership, this yields four
distributions per context, matching the sibling efferent analysis
``gawf_recurrent_group_gate_distributions`` in group semantics but with per-frame values
indexed by destination instead of source:

- ``within_top``    : destination in top10, source in top10 (within-top afferent).
- ``top_to_rem``    : destination in remaining, source in top10 (top -> rem afferent).
- ``rem_to_top``    : destination in top10, source in remaining (rem -> top afferent).
- ``within_rem``    : destination in remaining, source in remaining (within-remaining).

Outputs mirror the efferent module: one compressed NPZ plus JSON metadata for
recurrent_sector (9 levels) and recurrent_digit (10 levels).
"""
...
def accumulate_afferent_group_distributions(
    per_dest_from_top: np.ndarray,
    per_dest_from_rem: np.ndarray,
    contexts: np.ndarray,
    ...
    *,
    ...
    """Update histogram counts and moments for the four afferent (source, destination) groups.

    ``per_dest_from_top`` and ``per_dest_from_rem`` share shape ``(frames, H)`` and hold,
    for every frame, the mean afferent gate at each destination unit averaged over top-10%
    sources and remaining eligible sources respectively. ``contexts`` gives the per-frame
    context index; ``relevant_masks`` has shape ``(num_contexts, H)``; ``eligible`` is a
    ``(H,)`` boolean mask over hidden units. Returns updated ``hist``, ``sums``, ``sums_sq``,
    ``counts`` shaped ``(num_contexts, 4, ...)``.
    """

    per_dest_from_top = np.asarray(per_dest_from_top, dtype=np.float32)
    per_dest_from_rem = np.asarray(per_dest_from_rem, dtype=np.float32)
    if per_dest_from_top.shape != per_dest_from_rem.shape:
        raise ValueError("top and remaining per-destination arrays must share shape")
    if per_dest_from_top.ndim != 2:
        raise ValueError("per-destination arrays must be two-dimensional (frames, H)")
    contexts = np.asarray(contexts, dtype=np.int64).reshape(-1)
    if contexts.size != per_dest_from_top.shape[0]:
    ...
    if per_dest_from_top.shape[1] != eligible.size:
        raise ValueError("per-destination arrays must have H columns matching eligible")
    ...
    for context in np.unique(contexts):
        ...
        # Values are indexed by DESTINATION unit's group membership.
        top_dest_from_top_vals = per_dest_from_top[frame_selector][:, top_mask].reshape(-1)
        rem_dest_from_top_vals = per_dest_from_top[frame_selector][:, rem_mask].reshape(-1)
        top_dest_from_rem_vals = per_dest_from_rem[frame_selector][:, top_mask].reshape(-1)
        rem_dest_from_rem_vals = per_dest_from_rem[frame_selector][:, rem_mask].reshape(-1)
        # Order matches GROUP_NAMES: within_top, top_to_rem, rem_to_top, within_rem.
        # top_to_rem = source top, destination rem (afferent at rem dest from top src).
        # rem_to_top = source rem, destination top (afferent at top dest from rem src).
        group_values = (
            top_dest_from_top_vals,   # within_top
            rem_dest_from_top_vals,   # top_to_rem (source top -> dest rem)
            top_dest_from_rem_vals,   # rem_to_top (source rem -> dest top)
            rem_dest_from_rem_vals,   # within_rem
        )
...
def within_between_cohens_d(
    ...
    Returns a ``(num_contexts, 2)`` array. Column 0 compares ``within_top`` against
    ``rem_to_top`` (both terminate at top-10% destinations, so the pair isolates the effect
    of the source group for a top destination). Column 1 compares ``within_rem`` against
    ``top_to_rem`` (both terminate at remaining destinations). A positive value means the
    within-group afferent gate is stronger than the between-group afferent gate.
    """
...
def _accumulate_batch_frame(
    ...
    """Update every cell's afferent four-group buffers with one batch-frame recurrent gate."""

    import torch

    for cell, (_factor, _levels, label_col) in CELL_SPECS.items():
        ...
        for context_value in unique_contexts.tolist():
            ...
            # gate_recurrent shape: (B, H_dest, H_source). We want, per destination,
            # the mean over top-10% sources and remaining sources.
            # Sum over source axis with source-group weights, then divide by group counts.
            source_top_weights = top_mask_c.to(sub_gate.dtype)
            source_rem_weights = rem_mask_c.to(sub_gate.dtype)
            m_from_top = torch.einsum("i,bhi->bh", source_top_weights, sub_gate) / top_count
            m_from_rem = torch.einsum("i,bhi->bh", source_rem_weights, sub_gate) / rem_count
            top_dest_from_top = m_from_top[:, top_mask_c].reshape(-1).detach().cpu().numpy()
            rem_dest_from_top = m_from_top[:, rem_mask_c].reshape(-1).detach().cpu().numpy()
            top_dest_from_rem = m_from_rem[:, top_mask_c].reshape(-1).detach().cpu().numpy()
            rem_dest_from_rem = m_from_rem[:, rem_mask_c].reshape(-1).detach().cpu().numpy()
            group_values = (
                top_dest_from_top,   # within_top
                rem_dest_from_top,   # top_to_rem
                top_dest_from_rem,   # rem_to_top
                rem_dest_from_rem,   # within_rem
            )
            for group_index, values in enumerate(group_values):
...
def collect_and_accumulate(
    ...
    *,
    ...
    """Run reset trajectories once and accumulate every cell's afferent histograms."""
    ...
    return cell_buffers, bin_edges


def main() -> None:
    """Collect exact test gates and export the afferent four-group distributions."""
    ...
    cell_buffers, bin_edges = collect_and_accumulate(
        test_dataset,
        model,
        device,
        selections,
    ...
    for cell, (factor, _levels, _label_col) in CELL_SPECS.items():
        ...
        cell_metadata[cell] = {
            "gate": "recurrent gate, destination-oriented afferent unit mean over source subsets",
            "factor": factor,
            ...
            "within_top_cohens_d_vs_rem_to_top": cohens_d[:, 0].tolist(),
            "within_rem_cohens_d_vs_top_to_rem": cohens_d[:, 1].tolist(),
        }

    arrays_path = save_dir / "recurrent_afferent_group_gate_distributions.npz"
    metadata_path = save_dir / "recurrent_afferent_group_gate_distributions_meta.json"
    ...
    metadata = {
        ...
        "gate_values": "raw post-sigmoid; destination-oriented per-unit afferent means",
        "gate": (
            "for each destination unit h in a frame, two values are stored: "
            "mean over sources in top10(context) and mean over sources in "
            "remaining eligible units; destinations are then split by the same top10 mask"
        ),
        "hist_bins": args.hist_bins,
